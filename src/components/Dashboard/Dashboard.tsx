...
//   { icon: Calculator, label: "Visa Funds Calculator", path: "/funds", requiredPlan: "premium" },
...
//   const [currentPlan, setCurrentPlan] = useState<"standard" | "premium" | "enterprise">("standard")
...
//   // Function to check if a feature is available for the current plan
//   const isFeatureAvailable = (requiredPlan?: "standard" | "premium" | "enterprise") => {
//     if (!requiredPlan) return true // No plan requirement, feature is available to all


//     // Standard features are available to all plans
//     if (requiredPlan === "standard") return true

//     // Premium features are available to Premium and Enterprise plans
//     if (requiredPlan === "premium") {
//       return currentPlan === "premium" || currentPlan === "enterprise"
//     }

//     // Enterprise features are only available to Enterprise plan
...
//   // Function to refresh the user's plan status
...
//         // Update current plan based on stored plan
//         if (parsedPlan.name.toLowerCase().includes("enterprise")) {
//           setCurrentPlan("enterprise")
//           console.log("Plan refreshed to ENTERPRISE")
//         } else if (parsedPlan.name.toLowerCase().includes("premium")) {
//           setCurrentPlan("premium")
//           console.log("Plan refreshed to PREMIUM")
//         } else {
//           setCurrentPlan("standard")
//           console.log("Plan refreshed to STANDARD")
//         }
//       } catch (error) {
//         console.error("Error parsing stored plan during refresh:", error)
//       }
//     }
//   }
...
//       console.log("Plan status change detected")
//       refreshPlanStatus()
//     }

//     window.addEventListener("planStatusChanged", handlePlanStatusChange)
//     return () => {
//       window.removeEventListener("planStatusChanged", handlePlanStatusChange)
//     }
//   }, [])
...
//       // REPLACE this plan detection logic with the following improved version:
//       // Set the current plan - make this more robust by checking for substrings
//       if (parsedPlan.name.toLowerCase().includes("enterprise")) {
//         setCurrentPlan("enterprise")
//         console.log("Setting plan to ENTERPRISE from localStorage")
//       } else if (parsedPlan.name.toLowerCase().includes("premium")) {
//         setCurrentPlan("premium")
//         console.log("Setting plan to PREMIUM from localStorage")
//       } else {
//         setCurrentPlan("standard")
//         console.log("Setting plan to STANDARD from localStorage")
//       }
...
//             // Update current plan from Firestore data if available
...
//   // Function to get upgrade message based on required plan
//   const getUpgradeMessage = (requiredPlan: "standard" | "premium" | "enterprise") => {
//     if (requiredPlan === "premium") {
...
//   const handleNavItemHover = (e: React.MouseEvent, requiredPlan?: "standard" | "premium" | "enterprise") => {
//     if (requiredPlan && !isFeatureAvailable(requiredPlan)) {
//       const rect = e.currentTarget.getBoundingClientRect()
//       setTooltipInfo({
//         visible: true,
//         text: getUpgradeMessage(requiredPlan),
//         x: rect.right + 10,
...
//   const handleNavItemClick = (path: string, requiredPlan?: "standard" | "premium" | "enterprise") => {
//     // If feature is not available for current plan, don't navigate
//     if (requiredPlan && !isFeatureAvailable(requiredPlan)) {
//       return
//     }
...
//     // If navigating to account page, refresh plan status
//     if (section === "account") {
//       refreshPlanStatus()
//     }
...
//     // Check if the current section requires a higher plan
//     const currentNavItem = navigation.find((item) => item.path.replace("/", "") === activeSection)
//     if (currentNavItem?.requiredPlan && !isFeatureAvailable(currentNavItem.requiredPlan)) {
//       return (
//         <div className="p-8 flex flex-col items-center justify-center text-center">
//           <div className="bg-amber-100 p-6 rounded-xl max-w-md">
...
//               {currentNavItem.requiredPlan.charAt(0).toUpperCase() + currentNavItem.requiredPlan.slice(1)} plan or
//               higher.
//             </p>
//             <button
...
//                 const isAvailable = !action.requiredPlan || isFeatureAvailable(action.requiredPlan)

//                 return (
//                   <button
//                     key={action.label}
//                     onClick={() => (isAvailable ? setActiveSection(action.section) : null)}
//                     onMouseEnter={(e) =>
//                       action.requiredPlan && !isAvailable ? handleNavItemHover(e, action.requiredPlan) : null
//                     }
...
//                     </div>
//                     <span
//                       className={`font-medium text-sm sm:text-base ${isAvailable ? "text-gray-900" : "text-gray-500"}`}
//                     >
//                       {action.label}
//                     </span>
//                     {!isAvailable && <Lock className="w-4 h-4 text-gray-400 ml-auto" />}
//                   </button>
//                 )
//               })}
//             </div>
...
//               const isAvailable = !item.requiredPlan || isFeatureAvailable(item.requiredPlan)

//               return (
//                 <button
//                   key={item.path}
//                   onClick={() => handleNavItemClick(item.path, item.requiredPlan)}
//                   onMouseEnter={(e) =>
//                     item.requiredPlan && !isAvailable ? handleNavItemHover(e, item.requiredPlan) : null
//                   }
//                   onMouseLeave={handleNavItemLeave}
...
//                   {!isAvailable && <Lock className="w-4 h-4 ml-auto" />}
//                 </button>
//               )
//             })}
//           </nav>
...
//       {tooltipInfo.visible && (
//         <div
//           className="fixed bg-gray-800 text-white px-3 py-2 rounded-lg shadow-lg text-sm z-50"
//           style={{
...
//             style={{ left: "-4px", top: "50%", marginTop: "-4px" }}
//           />
//         </div>
//       )}
//       {/* Trial Banner */}
...
const Dashboard: React.FC = () => {
  ...
  const [showAdminPortal, setShowAdminPortal] = useState(false)
  const [trialDaysRemaining, setTrialDaysRemaining] = useState<number | null>(null)
  const { featureAccess, currentPlan } = useSubscriptionFeatures()
